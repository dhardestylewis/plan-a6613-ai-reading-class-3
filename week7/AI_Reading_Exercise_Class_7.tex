...
\noindent Google's efficiency claims circulate through a closed evidentiary loop: Google employees author the underlying research, Google cites it in sustainability reports as though it were independent, and AI tools reproduce those citations without flagging the self-referential chain. Breaking this loop requires the manual cross-referencing that AI tools cannot perform.
...
Google reports that its sixth-generation TPU, Trillium, is 67\% more energy-efficient than its predecessor, and that combining best practices can reduce training energy by 100 times and emissions by 1,000 times (Google, 2024). These figures originate from a 2022 paper whose authors are all Google employees, which predates the generative AI boom by seven months and whose claims are explicitly conditional on applying all practices simultaneously (Patterson et al., 2022). Google cites this paper in its own Environmental Reports without noting the affiliation, completing the first half of the evidentiary loop. Google's data centers achieve a power usage effectiveness of 1.10, compared with the industry average of 1.58 (Uptime Institute, 2023), and the median energy per Gemini text prompt fell 33-fold in twelve months (Google, 2025). Yet the IEA documents that combined electricity use by Amazon, Microsoft, Google, and Meta more than doubled between 2017 and 2021, reaching approximately 72 terawatt-hours, and continues to grow 20 to 40 percent annually (IEA, 2023). Per-unit efficiency gains, however real, are not keeping pace with absolute growth.

\section*{Verification and Reflection}
Claude completed the evidentiary loop. It correctly identified both Environmental Reports and cited emission figures accurately, but it reproduced Google's self-sourced efficiency claims as independent findings and defaulted to market-based accounting without surfacing the location-based alternative. Its limitation was \textit{framing}, not fabrication: it presented Google-authored, pre-GenAI, conditional claims as current, independently verified achievements. Google's emissions have risen 51\% since 2019 by its own market-based measure and 65\% by location-based accounting, yet the IEA's net-zero scenario requires data center emissions to \textit{halve} by 2030 (IEA, 2023). A planner using AI to evaluate these claims would inherit the closed loop intact. The source-first methodology broke the loop; the AI tool, left to its own framing, did not.
