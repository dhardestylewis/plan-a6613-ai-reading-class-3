...
\noindent AI research tools accept corporate sustainability framing by default. The gap between Google's efficiency narrative and its actual emissions trajectory only emerges through the kind of manual source cross-referencing that planners need but current AI tools cannot perform.
...
\textbf{\textit{2. Efficiency gains are real but overwhelmed by absolute growth.}}
Per-unit efficiency improvements are not keeping pace with the expansion of AI workloads. Google reports that its sixth-generation TPU, Trillium, is 67\% more energy-efficient than its predecessor, and that combining multiple best practices can reduce AI training energy by 100 times and associated emissions by 1,000 times (Google, 2024). These figures cite a 2022 paper whose authors are all Google affiliates and which predates the generative AI boom by seven months; its claims are explicitly conditional on applying all practices simultaneously (Patterson et al., 2022). Google's data centers achieve a power usage effectiveness of 1.10, compared with the industry average of 1.58 (Uptime Institute, 2023), and the median energy per Gemini text prompt fell 33-fold in twelve months (Google, 2025). Yet the IEA documents that combined electricity use by Amazon, Microsoft, Google, and Meta more than doubled between 2017 and 2021, reaching approximately 72 terawatt-hours, and continues to grow 20 to 40 percent annually in the hyperscale segment (IEA, 2023).

\section*{Verification and Reflection}
Claude correctly identified both Environmental Reports, cited emission figures accurately, and named specific mitigation technologies without hallucinating sources or fabricating statistics. Its limitation was \textit{framing}, not fabrication: it accepted Google's market-based accounting without surfacing the location-based alternative, and it presented conditional, pre-GenAI efficiency claims as current achievements. This matters because planners who rely on AI tools to evaluate corporate sustainability claims will inherit the corporate accounting frame without seeing it. Google's emissions have risen 51\% since 2019 by its own market-based measure and 65\% by location-based accounting, yet the IEA's net-zero scenario requires data center emissions to \textit{halve} by 2030 (IEA, 2023). The source-first methodology surfaced this contradiction; the AI tool, left to its own framing, did not.
